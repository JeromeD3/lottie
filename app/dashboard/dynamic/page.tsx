...
export default function Works17() {
  ...
  return (
    <MotionConfig transition={{ duration: 0.7, type: 'spring', bounce: 0.25 }}>
      <AnimatePresence>
        {open && <motion.div className="fixed inset-0 z-30 bg-white/30 backdrop-blur-lg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}></motion.div>}
      </AnimatePresence>
      <div className="fixed left-1/2 top-12 z-40 -translate-x-1/2">
        <motion.div
          ...
          className="bg-black relative cursor-pointer overflow-hidden text-white"
          style={{ borderRadius: 22 }}
        >
          <header className="flex h-11 cursor-pointer items-center gap-2 px-4" onClick={() => setOpen(!open)}>
            <ProgressCircle />
            <h1 className="grow font-bold">Nature Beauty</h1>
            <ScrollPercentage />
          </header>
